...
import eu.europa.ec.corelogic.di.isPresentationScopeCreated
...
@KoinViewModel
class IssuanceSuccessViewModel(
    ...
    override fun handleEvents(event: Event) {
        when (event) {
            ...
            is Event.PrimaryButtonPressed -> {
                val didPidIssuingHappenedDuringPresentation = isPresentationScopeCreated()
                when {
                    didPidIssuingHappenedDuringPresentation -> {
                        continueVerificationFlow()
                    }
                    else -> {
                        navigateToDashboard()
                    }
    ...
    private fun continueVerificationFlow() {
        setEffect {
            Effect.Navigation.SwitchScreen(
    ...
    private fun navigateToDocumentDetails() {
                    ...
                    screenRoute = generateComposableNavigationLink(
                        ...
                        arguments = generateComposableArguments(
                            mapOf(
                                ...
                                "documentType" to document.docType.toDocumentType().nameSpace,
                            )
                        )
                    )
                )
